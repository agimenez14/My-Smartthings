...
//	fingerprint deviceId: "0x2101", inClusters: " 0x70,0x31,0x72,0x86,0x32,0x80,0x85,0x60"
    ...
    // Watts row
		valueTile("powerDisp", "device.powerDisp", width: 2, height: 2) {
			state (
                ...
                backgroundColors:[
					...
					[value: "2000", 	color: "#7cff54"],
					[value: "4000", 	color: "#44b621"],
					[value: "8000", 	color: "#f1d801"],
					[value: "12000", 	color: "#d04e00"], 
					[value: "16000", 	color: "#bc2323"]
        ...
        valueTile("powerOne", "device.powerOne", width: 2, height: 2) {
        	state(
            	...
            	foregroundColor: "#000000",
                backgroundColors:[
					[value: "0 Watts", 		color: "#ffffff"],
					[value: "2000 Watts", 	color: "#90d2a7"],
					[value: "4000 Watts", 	color: "#90d2a7"],
					[value: "6000 Watts", 	color: "#44b621"],
	...
	// Power row
        ...
        valueTile("energyOne", "device.energyOne", width: 2, height: 2) {
        	state(
        		"default", 
        		label: '${currentValue}', 
        		foregroundColor: "#000000", 
    ...
    // Volts row
        ...
        valueTile("voltsOne", "device.voltsOne", width: 2, height: 2) {
        	state(
        		"default", 
        		label:'${currentValue}'
            )
        }
        valueTile("voltsTwo", "device.voltsTwo", width: 2, height: 2) {
        	state(
        		"default", 
        		label:'${currentValue}'
            )
    ...
    // Amps row
        valueTile("ampsDisp", "device.ampsDisp", width: 2, height: 2) {
        	state (
        		"default", 
        		label: '${currentValue}' , 
        		foregroundColor: "#000000", 
        ...
        valueTile("ampsOne", "device.ampsOne", width: 2, height: 2) {
        	state(
        		"default",
        		label:'${currentValue}',
        		foregroundColor: "#000000", 
        ...
        valueTile("ampsTwo", "device.ampsTwo", width: 2, height: 2) {
        	state(
        		"default", 
        		label:'${currentValue}',
        		foregroundColor: "#000000", 
    	...
    	valueTile("total", "device.total", width: 2, height: 2) {
        	state("default", label:'Total', foregroundColor: "#000000", backgroundColor: "#ffffff")
        }
    // Controls row
		standardTile("reset", "command.reset", inactiveLabel: false, width: 2, height: 2) {
		...
		standardTile("configure", "command.configure", inactiveLabel: false, width: 2, height: 2) {
...
// HEM Version Configuration only needs to be done here - comments to choose what gets displayed
		...
		main (["powerDisp","energyTwo",
			"ampsDisp","voltsDisp",				// Comment out this one for HEMv1
			"energyDisp"
			])
		details([
			"total","powerDisp","energyDisp",
            ...
            "energyTwo","powerOne","powerTwo",
			"energyOne","reset","refresh","toggle",
		...
		if (cmd.scale == 0) {
        	...
        	if (newValue != state.powerValue) {
                ...
                if (newValue < state.powerLow) {
                	...
                	if (state.display == 1) { sendEvent(name: "powerOne", value: dispValue as String, unit: "", descriptionText: "Low: ${newValue} Watts")	}
                    state.powerLow = newValue
                    state.powerLowDisp = dispValue
                }
                if (newValue > state.powerHigh) {
                	dispValue = newValue+" W\n"+timeString
                	if (state.display == 1) { sendEvent(name: "powerTwo", value: dispValue as String, unit: "", descriptionText: "High: ${newValue} Watts")	}
...
def resetDisplay() {
	...
	if ( state.display == 1 ) { 
        ...
        sendEvent(name: "ampsOne", value: "L1", unit: "")   
        sendEvent(name: "ampsDisp", value: state.powerL1Disp, unit: "")
    	sendEvent(name: "ampsTwo", value: state.energyL1Disp, unit: "")
    	sendEvent(name: "voltsOne", value: "L2", unit: "")
        sendEvent(name: "voltsDisp", value: state.powerL2Disp, unit: "")
    	sendEvent(name: "voltsTwo", value: state.energyL2Disp, unit: "")
	}
	else {
