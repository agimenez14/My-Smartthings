 ...
 */
metadata {
	definition (name: "Simulated Button", namespace: "RostikBor", author: "RostikBor") {
		capability "Actuator"
		capability "Button"
		capability "Sensor"
	...
	tiles {
		standardTile("button", "device.button", width: 1, height: 1) {
			state "default", label: "", icon: "st.alarm.beep.beep", backgroundColor: "#ffffff"
		}
 		...
 		standardTile("hold1", "device.button", width: 1, height: 1, decoration: "flat") {
			state "default", label: "Hold", backgroundColor: "#ffffff", action: "hold1"
		}          
		main (["push1"])
