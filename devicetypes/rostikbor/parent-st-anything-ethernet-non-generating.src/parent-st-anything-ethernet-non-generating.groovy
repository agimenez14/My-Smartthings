...
metadata {
	definition (name: "Parent_ST_Anything_Ethernet (non generating)", namespace: "RostikBor", author: "RostikBor") {
	...
	if (bodyString) {
		...
		try {
            ...
            if (childDevice != null) {
                ...
                /*if (name.startsWith("humidity")) {
                    //childDevice.typeName = "Child Temperature Sensor (w/ Humid)"
                    childDevice.deviceNetworkId = "${device.deviceNetworkId}-${temperature}"
                }   */
                childDevice.sendEvent(name: namebase, value: value)
                if (namebase.startsWith("motion") && value.startsWith("active")) {
                	def now = new Date().format("MMM-d-yyyy h:mm a", location.timeZone)
                	childDevice.sendEvent(name: "lastMotion", value: now)
                }
                if (namebase.startsWith("contact") && childDevice.typeName.startsWith("Child Contact Sensor (Water)")) 
                {
                	if (value == "open")
                ...
                if (namebase == "doorControl") {
                    ...
                    if (value.startsWith("open")) {
                        def now2 = new Date().format("MMM-d-yyyy h:mm a", location.timeZone)
                        childDevice.sendEvent(name: "lastOpen", value: now2)
                    }
                }
...
void childLevel(String dni, int level) {
    def name = dni.split("-")[-1]
    log.debug "childLevel($dni), name = ${name}"
